#!/usr/bin/env bash
#
# Pre-Edit Hook for gcp-project-operator
...
if [[ -z "$CANONICAL" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    # Use sys.argv to avoid shell injection via filenames with quotes
...
# Check for mockgen-generated files by path pattern
if [[ "$FILE" == pkg/util/mocks/* ]]; then
...
# Check for mockgen marker in file content (fallback for mocks outside standard paths)
if [[ -f "$REPO_ROOT/$FILE" ]] && head -1 "$REPO_ROOT/$FILE" 2>/dev/null | grep -q "Code generated by MockGen. DO NOT EDIT."; then
  echo "❌ BLOCKED: Cannot edit generated mock: $FILE"
  echo "   This file is auto-generated by mockgen."
  echo "   To regenerate: boilerplate/_lib/container-make generate"
  exit 1
...
if [[ "$FILE" == deploy/crds/* ]] && [[ "$FILE" == *.yaml ]]; then
  ...
  echo "   To regenerate CRDs: make op-generate"
  echo ""
