...
with open('config/templates/gcp-project-operator-csv-template.yaml', 'r') as stream:
...
# Add gcp-project-operator role to the CSV:
...
# Add gcp-project-operator-client role to the CSV:
...
csv['metadata']['name'] = "gcp-project-operator.v%s" % full_version
csv['spec']['version'] = full_version
csv['spec']['replaces'] = "gcp-project-operator.v%s" % prev_version
...
csv_filename = "gcp-project-operator.v%s.clusterserviceversion.yaml" % full_version
